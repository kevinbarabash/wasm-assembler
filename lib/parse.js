...
const processInsts = (insts, localIdx = []) => {
    ...
    while (i < insts.length) {
        ...
        } else if (inst in instructions) {
            code.push(instructions[inst])
        } else {
            if (Array.isArray(inst)) {
                throw new Error(`(${inst.join(' ')}) instruction not recognized`)
            } else {
                throw new Error(`${inst} instruction not recognized`)
            }
        }
    }
...
const processFunc = (func) => {
    ...
    if (funcIdx.indexOf(funcName) !== -1) {
        throw new Error(`func ${funcName} already defined`);
    }
...
const processImport = (exp) => {
    ...
    if (kind === 'func') {
        ...
        const params = func.filter(child =>
            Array.isArray(child) && child[0] === 'param')
        const results = func.filter(child =>
            Array.isArray(child) && child[0] === 'result')
...
const processExport = (exp) => {
    const index = funcIdx.indexOf(exp[2][1]);
    if (index === -1) {
        throw new Error(`export: function ${exp[2][1]} not defined`)
    }
    program.export.push({
        name: exp[1],
        type: exp[2][0],
...
const processStart = (expr) => {
    const name = expr[1]
    const index = funcIdx.indexOf(name);
    if (index === -1) {
        throw new Error(`start: function ${name} not defined`)
    }
    program.start.push({
        name: name,
        index: index,
